    ...
    <el-card>
      ...
      <dl>
        <template v-for="(val, key) in settings">
          <dt :key="key + 'a'">{{ optionDesc[key] ? optionDesc[key] + '('+ key + ')' :  key }}</dt>
          <dd :key="key + 'b'">
      ...
      <el-button @click="changeGlobalOption" type="primary">保存</el-button>
...
export default {
  ...
  methods: {
    async changeGlobalOption() {
      const data = await this.aria2.changeGlobalOption(this.settings)
      if(data !== 'OK') {
